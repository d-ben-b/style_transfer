...
pytorch_model.load_state_dict({k: v for k, v in state_dict.items() if not k.endswith(('running_mean', 'running_var'))}, strict=False)
...
def process_image(input_img, backend_choice):
    ...
    try:
        if backend_choice == "PyTorch (FP32)":
            ...
            if not trt_fp16_engine: return None, "找不到 mosaic_fp16.engine"
            out_img_chw, infer_time = trt_fp16_engine.infer(img_batch)

        elif backend_choice == "TensorRT (INT8)":
            if not trt_int8_engine: return None, "找不到 mosaic_int8.engine"
